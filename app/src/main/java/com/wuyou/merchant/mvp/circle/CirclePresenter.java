...
public class CirclePresenter extends CircleContract.Presenter {
    ...
    void getCreatedContract(String type) {
        CarefreeRetrofit.getInstance().createApi(CircleApis.class)
                .getContractList(CarefreeDaoSession.getInstance().getUserInfo().getShop_id(), QueryMapBuilder.getIns().put("start_id", "0")
                        .put("flag", "1")
                        .put("size", "10")
                        .put("type", type)
                        .buildGet())
    ...
    void loadListMore(String type) {
        CarefreeRetrofit.getInstance().createApi(CircleApis.class)
                .getContractList(CarefreeDaoSession.getInstance().getUserInfo().getShop_id(),
