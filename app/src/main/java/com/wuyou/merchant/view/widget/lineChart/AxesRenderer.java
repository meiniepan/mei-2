...
public class AxesRenderer {
    ...
    public AxesRenderer(Context context, Chart chart) {
        ...
        for (int position = 0; position < AXES_MUL; ++position) {
    ...
    private void prepareCustomAxis(Axis axis, int position) {
        ...
        if (axis.hasLines() && (linesDrawBufferTab[position].length < axis.getValues().size() * AXES_MUL)) {
            linesDrawBufferTab[position] = new float[axis.getValues().size() * AXES_MUL];
        }
    ...
    private void prepareAutoGeneratedAxis(Axis axis, int position) {
        ...
        FloatUtils.computeAutoGeneratedAxisValues(start, stop, Math.abs(contentRectDimension) /
                labelDimensionForStepsTab[position] / 4, autoValuesBufferTab[position]);
        //Reinitialize tab to hold lines coordinates.
        if (axis.hasLines()
                && (linesDrawBufferTab[position].length < autoValuesBufferTab[position].valuesNumber * AXES_MUL)) {
            linesDrawBufferTab[position] = new float[autoValuesBufferTab[position].valuesNumber * AXES_MUL];
        }
        ...
        if (axis.hasLines()) {
            ...
            for (; valueToDrawIndex < valuesToDrawNumTab[position]; ++valueToDrawIndex) {
                ...
                if (valueToDrawIndex == 0) {
                    linesDrawBufferTab[position][valueToDrawIndex * AXES_MUL + 0] = 0;
                    linesDrawBufferTab[position][valueToDrawIndex * AXES_MUL + 1] = 0;
                    linesDrawBufferTab[position][valueToDrawIndex * AXES_MUL + 2] = 0;
                    linesDrawBufferTab[position][valueToDrawIndex * AXES_MUL + 3] = 0;
                } else {
                    linesDrawBufferTab[position][valueToDrawIndex * AXES_MUL + 0] = lineX1;
                    linesDrawBufferTab[position][valueToDrawIndex * AXES_MUL + 1] = lineY1;
                    linesDrawBufferTab[position][valueToDrawIndex * AXES_MUL + 2] = lineX2;
                    linesDrawBufferTab[position][valueToDrawIndex * AXES_MUL + 3] = lineY2;
                }
            }
            canvas.drawLines(linesDrawBufferTab[position], 0, valueToDrawIndex * AXES_MUL, linePaintTab[position]);
        }
    }
